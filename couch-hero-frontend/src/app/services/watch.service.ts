...
export class WatchService {
  ...
  connect() {
    ...
    UART.connect((connection) => {
      if (!connection) {
        alert("Verbindung zur Bangle.js konnte nicht aufgebaut werden :(");
      }

      connection.write("reset();\n");

      setTimeout(_ => {
        this.uploadAndExecuteApp(connection);
      }, 500);
      connection.on('close', function () {
        alert("Verbindung wurde beendet");
      });
    });
  }
  uploadAndExecuteApp(connection) {
    const bangleCode= `
    Bangle.setHRMPower(true, "pulsapp");
    Bangle.on('HRM', function(hrm) {
      Bluetooth.println(hrm.bpm);
    });
    `;

    connection.write(bangleCode);

    setTimeout(()=> {
      connection.on('data', function(d) {
        document.getElementById("pulsAnzeige").innerHTML = d;
      });
    }, 500)
  }
